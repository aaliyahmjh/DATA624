...
clean_data <- function(df) {
  df %>%
    ...
    mutate(Brand_Code = as.numeric(as.factor(Brand_Code, ordered=FALSE))) %>%
    mutate(across(where(is.numeric), ~replace_na(., median(., na.rm = TRUE))))
}

train_data <- clean_data(train_raw) %>% drop_na(PH)  # Only drop rows missing the target
test_data <- clean_data(test_raw) %>% select(-PH)
...
These results guided our decision to ultimately choose **Random Forest** as the leading candidate for final model training and evaluation.
